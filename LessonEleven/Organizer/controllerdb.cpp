...
ControllerDB::ControllerDB() :
    QWidget(nullptr),
    ui(new Ui::ControllerDB)
{
    model = new QSqlRelationalTableModel{ui->tableTasksView, QSqlDatabase::addDatabase("QSQLITE")};
...
void ControllerDB::writeDB(QString taskName, QString deadline, QString progress)
{
...
int ControllerDB::getTasksSize()
{
    return numberOfTasks;
}

void ControllerDB::showTasks()
{
    ui->setupUi(this);
    ui->tableTasksView->setModel(model);
    model->setTable("tasks");
    model->select();
    this->show();
}

bool ControllerDB::checkDate(QString date)
{
    if(date.size() != 10)
    {
...
bool ControllerDB::checkTaskName(QString taskName)
{
...
bool ControllerDB::checkDay(int day, int month, int year, QDate currentDate)
{
...
bool ControllerDB::checkMonth(int month, int year, QDate currentDate)
{
...
bool ControllerDB::checkYear(int year, QDate currentDate)
{
    if(year < currentDate.year())
    {
